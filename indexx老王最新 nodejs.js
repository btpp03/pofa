...
const NEZHA_SERVER = process.env.NEZHA_SERVER || '';     // 哪吒面板地址,v1形式：nz.serv00.net:8008  v0形式：nz.serv00.
const NEZHA_PORT = process.env.NEZHA_PORT || 'nezhak2.btpp.ggff.net';         // v1哪吒请留空，v0 agent端口，当端口为{443,8443,2087,2083,2053,2096}时，自动开启tls
const NEZHA_KEY = process.env.NEZHA_KEY || '443';           // v1的NZ_CLIENT_SECRET或v0 agwnt密钥 
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'tAChwVSMAkeIqNUAVp';       // argo固定隧道域名,留空即使用临时隧道
...
const TUIC_PORT = process.env.TUIC_PORT || 40000;           // tuic端口，需配置frp3个参数开启
const HY2_PORT = process.env.HY2_PORT || 50000;             // hy2端口，需配置frp3个参数开启
...
const FRP_IP = process.env.FRP_IP || '210.131.221.81';                 // frp服务端公网ip
const FRP_PORT = process.env.FRP_PORT || '7000';         // frp服务端监听端口(通常为7000)
const FRP_TOKEN = process.env.FRP_TOKEN || 'TpCfGPIeHYVN37T4';           // frp服务端与客户端通信验证token
